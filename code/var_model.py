...
DESCRIPTION:
This script analyzes the sentiment polarity time series for Trump and Biden
tweets, tests for stationarity, fits a Vector AutoRegression (VAR) model,
...
if __name__ == "main":
    ...
    forecast = results.forecast(
        diff_data.values[-results.k_ar:], steps=forecast_steps)
    forecast_index = pd.date_range(
        start=diff_data.index[-1], periods=forecast_steps + 1, freq='24H')[1:]
